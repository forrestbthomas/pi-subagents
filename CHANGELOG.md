...
- Add opt-in `toolTimeoutMs` per-tool-call deadlines with call → agent → config → environment precedence. A wedged tool now terminates the child with `timedOut: true`; supervisor waits (`contact_supervisor`, `intercom`) remain exempt and a shorter run-level deadline wins. Thanks to @forrestbthomas for #1077.
