...
| `toolTimeoutMs` | Optional positive integer per-tool-call deadline in milliseconds. Off when omitted. An explicit call value wins, then this agent default, global `toolTimeoutMs`, and `PI_SUBAGENT_TOOL_TIMEOUT_MS`. It does not extend the run-level deadline; `contact_supervisor` and `intercom` are exempt. |
