...
| `toolTimeoutMs` | number | off | Optional positive per-tool-call deadline in milliseconds. Precedence: call value → agent frontmatter → config → `PI_SUBAGENT_TOOL_TIMEOUT_MS`. The timer starts on `tool_execution_start`, clears on `tool_execution_end`, and terminates the run with `timedOut: true` if the tool remains open. It never extends the run deadline; `contact_supervisor` and `intercom` are exempt. |
